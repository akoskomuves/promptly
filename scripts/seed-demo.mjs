...
const require = createRequire(path.join(process.cwd(), "packages/cli/node_modules/.pnpm"));
// Resolve better-sqlite3 from the cli package
const dbModPath = path.join(process.cwd(), "node_modules/.pnpm/better-sqlite3@11.10.0/node_modules/better-sqlite3/lib/index.js");
const Database = (await import(dbModPath)).default;
...
db.exec("DELETE FROM sessions WHERE ticket_id LIKE 'AUTH-%' OR ticket_id LIKE 'UI-%' OR ticket_id LIKE 'API-%' OR ticket_id LIKE 'FIX-%' OR ticket_id LIKE 'FEAT-%' OR ticket_id LIKE 'PERF-%' OR ticket_id LIKE 'REFACTOR-%'");
...
for (let daysAgo = 30; daysAgo >= 0; daysAgo--) {
  ...
  const sessionsToday = isWeekday ? rand(2, 5) : rand(0, 2);

  for (let i = 0; i < sessionsToday; i++) {
    const tool = pick(tools);
    const pool = modelMap[tool];
    const sessionModels = Math.random() > 0.7 ? pool : [pick(pool)];
    const prefix = pick(ticketPrefixes);
    const name = pick(ticketNames[prefix]);
    const ticketId = `${prefix}-${rand(100, 999)}-${name}`;

    const startDate = new Date(date);
    startDate.setHours(rand(8, 20), rand(0, 59), rand(0, 59));

    const durationMin = rand(3, 90);
    ...
    const conversations = [];
    const turnCount = Math.min(messageCount, 12);
    for (let t = 0; t < turnCount; t++) {
      const role = t % 2 === 0 ? "user" : "assistant";
      const content = role === "user" ? pick(userMsgs) : pick(assistantMsgs);
      const turnTime = new Date(startDate.getTime() + (t / turnCount) * durationMin * 60000);
      const turn = { role, content, timestamp: turnTime.toISOString(), model: pick(sessionModels), tokenCount: rand(50, 2000) };
    ...
    insert.run(
      genId(), ticketId, startDate.toISOString(),
      isActive ? null : endDate.toISOString(),
      isActive ? "ACTIVE" : "COMPLETED",
      promptTokens + responseTokens, promptTokens, responseTokens,
      messageCount, toolCallCount,
      JSON.stringify(conversations), JSON.stringify(sessionModels),
